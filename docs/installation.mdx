...
the XPipe Desktop application, which features a full-fledged GUI.
However, the different ways to use XPipe don't stop there.
XPipe comes with APIs for many programming languages, hopefully also your favorite programming language.
It also includes support through addons and integrations for select third-party applications.
The XPipe project consists out of several components that work together:

##### XPipe Daemon
The XPipe daemon is the core component that handles all operations in the background.

##### XPipe Desktop
The XPipe Desktop application provides a GUI with which you can access the full XPipe functionality.

##### Command-line tools
To access most of the XPipe functionality from the command-line, there exists the `xpipe` / `xpipe.exe` CLI executable,
which is included in all XPipe distributions.

##### XPipe APIs
The XPipe APIs that are available for various programming languages allow you to interact with XPipe in your program or script.
These APIs have to be added to your application manually, they do not ship with your XPipe installation.

##### Application integrations
There also exist special integrations for select third-party applications.
These integrations also have to be installed manually, they do not ship with your XPipe installation.
